  ...
  return (
    <div className="pt-3">
      <div className="flex justify-end mb-3">
            ...
            onClick={() => {
              if (type === CSS) {
                const exitAnimation = async () => {
                  await animateCss(scopeCSS.current, { height: 0, opacity: 0 });
                };
                setType(null);
                exitAnimation();
                return;
              ...
              if (type === JS) {
                const animation = async () => {
                  await animateJS(scopeJS.current, { height: 0, opacity: 0 }, { duration: 0 });
                  await animateCss(scopeCSS.current, { height: 'auto', opacity: 1 }, { duration: 0 });
                };
                animation();
                setType(CSS);
                return;
              ...
              const enterAnimation = async () => {
                await animateCss(scopeCSS.current, { height: 'auto', opacity: 1 });
              };
              enterAnimation();
              setType(CSS);
            }}
        ...
        <div
          ...
          onClick={() => {
            if (type === JS) {
              const exitAnimation = async () => {
                await animateJS(scopeJS.current, { height: 0, opacity: 0 });
              };
              setType(null);
              exitAnimation();
              return;
            ...
            if (type === CSS) {
              const animation = async () => {
                await animateCss(scopeCSS.current, { height: 0, opacity: 0 }, { duration: 0 });
                await animateJS(scopeJS.current, { height: 'auto', opacity: 1 }, { duration: 0 });
              };
              animation();
              setType(JS);
              return;
            ...
            const enterAnimation = async () => {
              await animateJS(scopeJS.current, { height: 'auto', opacity: 1 });
            };
            enterAnimation();
            setType(JS);
          }}
      ...
      <LazyMotion features={domAnimation}>
        <AnimatePresence>
          <m.div className="overflow-auto max-h-96 rounded" ref={scopeJS} initial={{ height: 0 }} key={1}>
            <JsPreview code={code} onChange={onChange} />
          </m.div>
          {cssCode && (
            <m.div className="overflow-auto max-h-96 rounded" ref={scopeCSS} initial={{ height: 0 }} key={2}>

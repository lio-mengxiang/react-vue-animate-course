import React from 'react';
import { AnimationA } from './animationA';
import { AnimationB } from './animationB';
import { AnimationC } from './animationC';

import { type TitleProps } from './interface';

import './title.css';

export function Title({ state }: TitleProps) {
  return (
    <div className="title">
      {state === 'A' && <AnimationA />}
      {state === 'B' && <AnimationB />}
      {state === 'C' && <AnimationC />}
    </div>
  );
}

// <svg className="w-[4vh] mt-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="46" height="46" color="color-1" fill="none">
//   <path
//     d="M12 22C18 22 19.5 17.49 19.5 12C19.5 6.50998 18 2 12 2C5.99993 2 4.5 6.50996 4.5 12C4.5 17.49 5.99993 22 12 22Z"
//     stroke="currentColor"
//     strokeWidth="1.5"
//   ></path>
//   <g stroke="currentColor" className="mouse-up-down">
//     <path d="M11.9883 6.83859L11.9883 11.7586" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
//     <path
//       d="M9.99609 7.8586C10.9801 6.8386 11.5801 5.9386 12.0361 6.00329C12.4201 6.0001 12.7801 6.5986 14.0041 7.8586"
//       strokeWidth="1.5"
//       strokeLinecap="round"
//       strokeLinejoin="round"
//     ></path>
//     <path
//       d="M14.0041 11.1406C13.0201 12.1606 12.4201 13.0606 11.9641 12.9959C11.5801 12.9991 11.2201 12.4006 9.99609 11.1406"
//       strokeWidth="1.5"
//       strokeLinecap="round"
//       strokeLinejoin="round"
//     ></path>
//   </g>
// </svg>;
